"""将 sub_test/C++.jsonl 中的 buggy C++ 编译为 LLVM IR，写入 LLVM IR.jsonl 与 LLVM IR/ 目录。"""
...
DEFAULT_CPP_JSONL = os.path.join(_DIR, "sub_test", "C++.jsonl")
DEFAULT_OUT_JSONL = os.path.join(_DIR, "sub_test", "LLVM IR.jsonl")
DEFAULT_OUT_DIR = os.path.join(_DIR, "sub_test", "LLVM IR")

# 竞赛代码常见非标准用法：gets、缺省 main 返回类型等
_COMPILE_PREAMBLE = """\
#line 1 "apr_snippet.cpp"
#include <cstdio>
#include <cstring>
static char* apr_gets(char* s) { return std::fgets(s, 1 << 20, stdin); }
...
def compile_cpp_to_llvm(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="apr_llvm_") as tmp:
        src = os.path.join(tmp, "snippet.cpp")
...
def convert_dataset(
    *,
    ...
    for index, item in enumerate(_iter_jsonl(cpp_jsonl)):
        stats["total"] += 1
        ...
        if not isinstance(code, str) or not code.strip():
            ...
            ir_text, status = compile_cpp_to_llvm(code, clang=clang)
...
def main() -> int:
    ...
    print(
        f"完成: total={stats['total']} ok={stats['ok']} failed={stats['failed']}"
    )
